 ...
 * J2EE平台在接受用户创建任务的请求之后，会将任务信息插入MySQL的task表中，任务参数以JSON格式封装在task+param
 * 字段中
 * 
 * 接着J2EE平台会执行我们的spark-submit shell脚本，并将taskid作为参数传递给spark-submit shell脚本
 * spark-submit shell脚本，在执行时，是可以接受参数的，并且会将接受的参数，传递给spark作业的main函数
...
public class UserVisitSessionAnalyzeSpark {
	...
	public static void main(String[] args) {
		...
		SparkConf conf = new SparkConf()
				...
				//.set("spark.default.parallelism", "100") //一般设置为spark-submit中总的cpu core的2——3倍
				.set("spark.storage.memoryFraction", "0.5")//进行persist内存占比
				.set("spark.shuffle.consolidateFiles", "true")
				.set("spark.shuffle.file.buffer", "64")//调节map task 内存缓冲大小，这个缓冲在写入磁盘之前。
				.set("spark.shuffle.memoryFraction", "0.3")//reduce 端task 后面，内存缓冲HashMap（0.2）对key的value执行聚合操作，然后spill磁盘。这里指的是内存缓冲
				.set("spark.reducer.maxSizeInFlight", "24") //reduce 拉取map端数据的缓冲区大小，最大拉取多少，默认64，设置少了，就多拉去几次，可以减少reduce端内存的压力，避免OOM
				.set("spark.shuffle.io.maxRetries","60")//JVM GC 导致的shuffle file not found 
				.set("spark.shuffle.io.retryWait", "60")//JVM GC 导致的shuffle file not found 
		...
		/**
		 * 比如，二次排序的Key是需要在进行shuffle的时候，进行网络传输的，因此要求实现序列化的
		 * 启用Kyro机制以后，就会要Kryo去序列化和反序列化CategorySortKey
		 * 所以这里需要注册一下我们自己定义的类
		 */
		...
		/**
		 * actionRDD，就是一个公共RDD
		 * 第一，要用actionRDD，获取一个公共的sessionid为key的PairRDD
		...
		//与用户信息数据，进行join
		//然后就可以获得session粒度的数据，同时呢，数据里面还包含了session对应的user信息
		//到这里为止，获取的数据是《sessionid，（session，searchKeywords，clickCategoryIds，age，professional，city，sex）》
		JavaPairRDD<String, String> sessionid2AggrInfoRDD = 
				aggregateBySession(sc,sqlContext,session2actionRDD);
		
		
		//接着，就要针对session粒度的聚合数据，按照使用者指定的刷选参数进行数据过滤
		...
		//生成公共的RDD，通过刷选条件的session的访问明细数据
		
		/**
		 * 重构，sessioniddetailRDD，就是代表了通过筛选的session对应的访问明细数据
		 */
		JavaPairRDD<String, Row> sessionid2detailRDD = getSessionid2detailRDD(
				filteredSessionid2AggrInfoRDD, session2actionRDD);
		sessionid2detailRDD = sessionid2detailRDD.persist(StorageLevel.MEMORY_ONLY());
		
		/**
		 * 对于Accumulator这种分布式累加计算的变量的使用，
		 * 1，从Accumulator中，获取数据，插入到数据库的时候，一定要是在某一个action操作以后在进行。如果没有action，那么整个程序根本不会运行，
		 * 必须把最终触发job执行的操作，放在最终写入MySQL方法之前
		 */
		 ...
		 * 所以，我们在这里，将随机抽取的功能的实现代码，放在session聚合统计功能的最终计算和写库之前（放到获取Accumulator.value之前）
		 ...
		 * 4，使用自定义Accumulator中的统计量，去计算各个区间的比例
		 * 5，将最后计算出来的结果，写入MySQL对应的表中
		 * 
		 * 普通实现思路的问题：
		 * 1，为什么还要用actionRDD？其实我们之前在session聚合的时候，映射已经做过了。多次一举
		 * 2，是不是一定要，为了session的聚合这个功能，单独去遍历一遍session？其实没有必要，已经有session数据了
		 * 		之前过滤session的时候，其实，就相当于在遍历session，这里就没有必要再过滤一遍了
		 * 
		 * 重构实现思路：
		 * 1，不要去生成任何新的RDD
		 ...
		 * 
		 * 尽量少生成RDD
		 * 尽量少对RDD进行算子操作，如果有可能，尽量子啊一个算子里面，实现需要做的多个功能。
		 * 尽量少对RDD进行shuffle算子操作。 shuffle操作，会导致大量的磁盘读写，严重降低性能。
		 * 
		 */
	...
	/**
	 * 获取SQLContext
	 * 如果是在本次测试环境的话，那么就生成SQLContext对象
	 * 若果是在生产环境运行的话，那么就生成HiveContext对象
	 * @param sc
	 * @return
	 */
...
//		});
		...
		return actionRDD.mapPartitionsToPair(new PairFlatMapFunction<Iterator<Row>, String, Row>() {
			...
			public Iterable<Tuple2<String, Row>> call(Iterator<Row> iterable) throws Exception {
				...
				new PairFunction<Tuple2<String,Iterable<Row>>, Long, String>() {
					...
					public Tuple2<Long, String> call(Tuple2<String, Iterable<Row>> tuple) throws Exception {
						...
						while(iterator.hasNext()) {
							...
							//所有，任何一行行为数据，都不可能两个字段都有，所有数据是可能出现null的
						...
						//这里返回的数据格式：《sessionid，partAggrInfo》
						//但是，这一步聚合完了以后，其实，我们是还需要将每一数据，跟对应的用户信息进行聚合
						//问题就来了，如果是跟用户信息进行聚合的话，那么key，就不应该是sessionid
						//就应该是userid，才能够给<userid,Row>格式的用户信息进行绝活
						//如果我们这里直接返回《sessionid，partAggrInfo>，还需要做一次mapTopair算子
						//将RDD映射成《userid，partAggrInfo>的格式，那么就多次一举
						
						//所以，我们致力其实可以直接，返回的数据格式，就是《userid，partAggrInfo》
						//然后跟用户信息join的时候，将partAggrInfo关联上userInfo
						//然后再直接将返回的Tuple的key设置成sessionid
						//最后的数据格式，还是《session，fullAggrinfo>
...
//				});
		...
		JavaPairRDD<String, String> filteredSessionid2AggrInfoRDD = sessionid2AggrInfoRDD.filter(
				...
				new Function<Tuple2<String,String>, Boolean>() {
					...
					public Boolean call(Tuple2<String, String> tuple) throws Exception {
						...
						//如果经过了之前的多个过滤条件之后，程序走到这里，
						//那么就说明，该session是通过了用户指定的筛选条件
						//那么对session的访问时长和访问步长进行统计
	...
	/**
	 * 随机抽取session
	 * @param sessionid3AggrInfoRDD
	 */
	private static void randomExtractSession(
			JavaSparkContext sc,
			final long taskid,
			JavaPairRDD<String, String> sessionid3AggrInfoRDD,
			JavaPairRDD<String, Row> session2actionRDD) {
		//第一步，计算出每天每小时的session数量,获取<yyyy-MM-dd_HH,sessionid>格式的RDD
		JavaPairRDD<String, String> time2sessionidRDD = sessionid3AggrInfoRDD.mapToPair(
		...
		/**
		 * session随机抽功能
		 * 用到了一个比较大的便令，随机抽取索引map
		 * 之前是直接在算子里面使用了这个map，每个task都会拷贝一份map副本，还是比较消耗内存和网络性能的。
		 * 
		 * 将map做成广播变量
		 */
		...
		for(Map.Entry<String,Map<String,Long>> dateHourCountEntry : dateHourCountMap.entrySet()) {
			...
			for(Map.Entry<String, Long> hourCountEntry : hourCountMap.entrySet()) {
				...
				//计算每个小时的session树龄，占据当天总session树龄的比例，直接乘以每天要抽取的数量
		...
		//得到<dataHour,（session aggrInfo）>
		JavaPairRDD<String, Iterable<String>> time2sessionsRDD = time2sessionidRDD.groupByKey();
		
		//我们用flatMap算子，遍历所有的<dataHour,（session aggrInfo）>格式的数据
		//然后，会遍历每天每小时的session
		///如果发现某个session恰巧在我们制定的这天这个小时的随机抽取索引上
		//那么抽取该session，直接写入MySQL的random_extract_session表
		//将抽取出来的session_id返回回来，形成一个新的JavaRDD<String>
		//然后最后一步，使用抽取出来的sessionid，去join它们的访问行为明细数据，写入session_detail表
		
		JavaPairRDD<String,String> extranctSessionidsRDD = time2sessionsRDD.flatMapToPair(
		...
		JavaPairRDD<String, Tuple2<String, Row>> extractSessionDetailRDD = 
				extranctSessionidsRDD.join(session2actionRDD);
...
//		});
	...
	private static List<Tuple2<CategorySortKey, String>> getTop10Category(
		 ...
		 */
		 ...
		 //获取session访问过的所有品类id,因为有可能一条记录对多个品类下单或者支付，所以用flatMap
		 ...
		 //分别来计算各品类点击，下单和支持的次数，可以先对访问明细进行过来吧
		 //分别过滤出点击，下单和支付，然后通过map.reduceByKey等算子来进行计算
		 ...
		 * 
		 * categoryidRDD中，是包含了所有的符合条件的session访问过的品类id
		 * 
		 * 上面分别计算出来的三份各类别的点击，下单和支付次数，可能不是包含所有品类的
		 * 比如，有的品类，就只是被点击过，但是没有人下单和支付
		 * 
		 * 所有，这里，就不能使用join操作，要使用leftOuterJoin操作，就是说，如果categoryidRDD不能join到自己的
		 * 某个数据，比如点击，或下单，或支付次数，那么该categoryidRDD还是要保留下来的
	 ...
	 */
	 private static JavaPairRDD<Long, Long> getClickCategoryId2CountRDD(
		 ...
		 /**
		  * 过滤，过滤出来点击行为的数据
		  * 点击行为的数据其实只占总数据量的一小部分
		  * 所以活了以后的RDD，每个partition的数据量，很有可能跟我们之前说的一样，会很不均匀
		  * 而且数据量肯定会变化很多
		  * 
		  * 所以针对这种情况，还是比较合适用一下coalesce算的，在filter火候去减少partition的数量
		  */
